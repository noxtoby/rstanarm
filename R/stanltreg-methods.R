...
#' Methods for stanltreg objects
#' 
#' S3 methods for \link[=stanreg-objects]{stanltreg} objects. There are also 
...
#' 
#' @name stanltreg-methods
#' 
#' @templateVar stanltregArg object,x
#' @templateVar mArg m
...
#' in \code{\link{print.stanltreg}} for more details. \code{coef} returns a list 
...
#' \code{\link{print.stanltreg}} for more details.
#' }
#' \item{\code{confint}}{
...
#' @seealso
#' \itemize{
#'  \item The \code{\link[=print.stanltreg]{print}},
#'    \code{\link[=summary.stanltreg]{summary}}, and \code{\link{prior_summary}} 
#'    methods for \code{stanltreg} objects for information on the fitted model.
...
#'    submodel (for models estimated using \code{\link{stan_ltjmm}}).
#'  \item The \code{\link{posterior_interval}} for uncertainty intervals for 
#'    model parameters.
#'  \item The \code{\link[=loo.stanreg]{loo}}, 
#'    and \code{\link[=log_lik.stanltreg]{log_lik}} methods for leave-one-out 
...
#' } 
#' 
#' Other S3 methods for stanltreg objects, which have separate documentation, 
#' including \code{\link{print.stanltreg}}, and \code{\link{summary.stanltreg}}.
#' 
...
#' @rdname stanltreg-methods
#' @export
#'    
coef.stanltreg <- function(object, m = NULL, ...) {
  M <- get_M(object)
  if (length(list(...))) 
...
#' @export
#' 
fitted.stanltreg <- function(object, m = NULL, ...)  {
...
#' @rdname stanltreg-methods
#' @export 
residuals.stanltreg <- function(object, m = NULL, ...) {
  stop("Not currently implemented.")
  M <- get_M(object)
  stub <- get_stub(object)
  if (is.null(m)) 
...
#' @rdname stanltreg-methods
#' @export
se.stanltreg <- function(object, m = NULL, ...) {
  stop("Not currently implemented.")
  M <- get_M(object)
  stub <- get_stub(object)
...
#'   part of the longitudinal submodel formulas  
formula.stanltreg <- function (x, fixed.only = FALSE, random.only = FALSE, m = NULL, ...) {
  ...
  form <- x$formula
  if (is.null(form))
    stop2("Could not find formula in stanltreg object.")
  if (fixed.only) {
    for (i in 1:M)
...
#' terms method for stanltreg objects
#' @export
#' @keywords internal
#' @templateVar mArg m
#' @template args-m
...
#' 
terms.stanltreg <- function(x, fixed.only = TRUE, random.only = FALSE, m = NULL, ...) {
  if (!is.stanltreg(x))
  ...
  if (is.ltjmm(x)) {
    M <- get_M(x)
    ltjmm_terms <- fetch(x$glmod, "terms")
    if (fixed.only) {
      Terms <- lapply(seq(M), function(i) {
        fe_form <- formula.stanltreg(x, fixed.only = TRUE, m = i)
        tt <- terms.formula(fe_form)
        attr(tt, "predvars") <- attr(ltjmm_terms[[i]], "predvars.fixed")
        tt
      })     
    } else if (random.only) {
      Terms <- lapply(seq(M), function(i) {
        re_form <- formula.stanltreg(x, random.only = TRUE, m = i) 
        tt <- terms.formula(lme4::subbars(re_form))
        attr(tt, "predvars") <- attr(ltjmm_terms[[i]], "predvars.random")
        tt
      })      
    } else {
      Terms[1:M] <- ltjmm_terms
    }
...
#'   \code{formula} argument in \code{\link{stan_ltjmm}}.
#' @param evaluate See \code{\link[stats]{update}}.
#'
update.stanltreg <- function(object, formula., ..., evaluate = TRUE) {
  call <- getCall(object)
  M <- get_M(object)
  if (is.null(call)) 
  ...
  if (!missing(formula.)) {
    ...
    fm_ltjmm <- lapply(1:M, function(m) 
      update.formula(fm[[m]], formula.[[m]]))
    names(fm_ltjmm) <- NULL
    fm_ltjmm <- as.call(c(quote(list), fm_ltjmm))
    call$formula <- fm_ltjmm
  }  
  if (length(extras)) {
...
#' 
fixef.stanltreg <- function(object, m = NULL, remove_stub = TRUE, ...) {
  M <- get_M(object)
...
#' 
ngrps.stanltreg <- function(object, ...) {
  object$n_grps  
}
...
#'
ranef.stanltreg <- function(object, m = NULL, ...) {
  M <- get_M(object)
  stub <- get_stub(object)
...
#'
sigma.stanltreg <- function(object, m = NULL, ...) {
  stub <- get_stub(object)
  if (is.null(m)) {
...
#' family method for stanltreg objects
#'
#' @keywords internal
#' @export
#' @templateVar mArg m
...
family.stanltreg <- function(object, m = NULL, ...) {
  M <- get_M(object)
  stub <- get_stub(object)
...
#' model.frame method for stanltreg objects
#' 
#' @keywords internal
#' @export
#' @templateVar mArg m
...
#' 
model.frame.stanltreg <- function(formula, fixed.only = FALSE, m = NULL, ...) {
  if (is.stanltreg(formula)) {
    M <- get_M(formula)
...
.stanltreg_check <- function(object) {
  if (!is.stanltreg(object))
    stop("This method is for stanltreg objects only.", call. = FALSE)
}
.cnms.stanltreg <- function(object, m = NULL, remove_stub = FALSE, ...) {
  .stanltreg_check(object)
  cnms <- if (is.null(m)) object$cnms else object$glmod[[m]]$reTrms$cnms
  if (remove_stub) lapply(cnms, rm_stub) else cnms
}
.flist.stanltreg <- function(object, m = NULL, ...) {
  .stanltreg_check(object)
  if (is.null(m)) {
    stop("'m = NULL' cannot currently be handled by .flist.stanltreg method.")
